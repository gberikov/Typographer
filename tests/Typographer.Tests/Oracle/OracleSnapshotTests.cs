...
public class OracleSnapshotTests
{
    ...
    private static readonly Dictionary<string, string> Divergences = new(StringComparer.Ordinal)
    {
        ...
        ["Минус -5 градусов и 5-й дом"] =
            "**пропуск:** правила минуса в реестре нет — дефис и минус в тексте неразличимы без знания замысла автора",
        ["Температура 25 C и угол 90 град."] =
            "знак градуса мы дописываем сами (ГОСТ 8.417), оракул нет; **пропуск:** неразрывный пробел перед «°C» не ставится",
        ["Дробь 1/2, 3/4 и 1/4"] =
        ...
        ["Разряды 1000000 и 1 000 000 рублей"] =
            "разряды мы разбиваем сами (common/number/digitGrouping в пресете), оракул нет; **пропуск:** уже стоящие пробелы между разрядами неразрывными не становятся",
        ...
        ["Деньги 100 руб., 50 долл., 5 000 р."] =
            "**пропуск:** уже стоящие пробелы между разрядами неразрывными не становятся",
        ...
        ["Проценты 50 % и 100%"] =
            "**пропуск:** в пресете Lebedev пробел перед процентом сохраняется, но неразрывным не становится",
