...
(омонимичные частицы), `en-GB/dash/main` и `en-US/dash/main`.

Причина одна: правило, меняющее смысл или запись текста, не должно срабатывать у
того, кто просто вызвал `Typograf.Html(text)`.

Зарегистрированы, но пока НЕ РЕАЛИЗОВАНЫ (следующий план): `common/html/*` — восемь правил
фаз `Layout` и `Emit`, — и `ru/optalign/*` — три правила висячей пунктуации, требующие CSS на
стороне сайта. Правило `ru/typo/switchingKeyboardLayout` зарегистрировано и тоже не
реализовано. Их XML-комментарии об этом говорят прямо.
