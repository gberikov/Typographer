...
Замер от 10 сентября 2026 — `docs/perf.md`. Второе выполнено, первое нет: получается
28 млн символов в секунду при закрытом реестре из 107 правил. Цифра 50 назначалась до
того, как стало известно, сколько правил в наборе, и решение — ускорять конвейер или
пересматривать цифру — пока не принято.
...
конвейер с двенадцатью правилами не может быть с ним одного порядка.
