...
- [ ] **Step 2: Прогнать тесты** — Run: `dotnet test`, Expected: FAIL по всем четырём.
- [ ] **Step 3: Реализовать** — ветви скобок в `SpaceRules.TryApply`; пробел перед открывающей скобкой дописывается только если слева буква или цифра, иначе `слово ( текст` даст два пробела.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: пробелы вокруг круглых и квадратных скобок`.
...
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL, класса `DocumentSpaceRules` нет.
- [ ] **Step 3: Реализовать** проход по образцу `LayoutWriter.WriteBreaks`: `MarkupScanner` по документу, обработка только `SegmentKind.Text`, разметка и защищённые зоны — байт в байт.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: нормализация пробелов и переводов строк на уровне документа`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице, плюс `Двойная!! и ??` остаётся без изменений при выключенном `ru/punctuation/exclamation`.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать** ветвь в `PunctuationRules.TryApply`: если текущий символ равен предыдущему в буфере (не левее `floor`) и входит в набор `,` `;` `:` `.`, символ не пишется. Точка требует осторожности: три точки уже схлопывает `hellip`, поэтому `delDoublePunctuation` не должен трогать точки вовсе — иначе многоточие перестанет собираться. Ограничь набор `,` `;` `:`.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS, все прежние тесты многоточия зелёные.
- [ ] **Step 5: Коммит** — `feat: удаление двойной пунктуации`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице плюс `10 Cm` → `10 Cm` (правило не сработало) и `a<-b` внутри текста.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать** `SymbolRules.TryApply` с ветвями `(`, `-`, `<`, `C`, `F`, `№`. Ветвь `-` в диспетчере уже занята `DashRules`: вызывай `SymbolRules` первым и только для случая `->`, иначе тире.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: типографские символы — копирайт, стрелки, градусы, знак номера`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице плюс `5 хорошо` (кириллическая `х` цела) и `1/2 текста` (дробь только между цифрами).
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать** `NumberRules.TryApply`. Дроби — только для трёх пар из таблицы: остальные дроби в Unicode неполны, и `5/7` оставлять как есть честнее, чем выдумывать.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: дроби, знаки сравнения, знак умножения и разряды чисел`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице, плюс `Шарль де Голль` без изменений и `кто то` внутри `<code>` без изменений.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать** ветвь пробела в `DashRules`: увидев пробел, посмотреть слово справа; если оно из списка частиц и слово слева не пусто, записать дефис вместо пробела.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: дефис в частицах «то», «либо», «нибудь», «ка», «таки», «кое»`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице, плюс `10:00-11:00` внутри ссылки не трогается и `XIX-XX` без «вв.» тоже становится тире (римские цифры сами по себе достаточный признак).
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать** словари и три правила.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: тире в веках, десятилетиях и интервалах времени`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL для первых трёх, PASS для `surname` (сейчас `ru/dash/main` и так не трогает дефис внутри слова — тест закрепляет).
- [ ] **Step 3: Реализовать**.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: тире между месяцами, днями недели и днями одного месяца`.
...
- [ ] **Step 3: Реализовать**.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: русская пунктуация — «?..», «!..», «?!» и запятые перед «а» и «но»`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице плюс два случая-исключения.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать**.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: десятичная запятая, порядковые числительные, пробел после многоточия и перед «год»`.
...
- [ ] **Step 1: Написать падающие тесты** — по таблице плюс тест на конфликт.
- [ ] **Step 2: Прогнать тесты** — Expected: FAIL.
- [ ] **Step 3: Реализовать**.
- [ ] **Step 4: Прогнать тесты** — Expected: PASS.
- [ ] **Step 5: Коммит** — `feat: английское тире en-GB и en-US`.
...
- [ ] **Step 2: Прогнать тест** — Expected: FAIL, пресеты сейчас совпадают с `Default`.
- [ ] **Step 3: Развести пресеты** и поправить их XML-комментарии: сейчас там написано «совпадает с Default, разойдётся в следующей версии».
- [ ] **Step 4: Пополнить корпус** — добавить в `HardCases.All` по одному входу из каждой группы правил этого плана; корпус гоняется тестами идемпотентности и целостности разметки, то есть новые правила автоматически проверяются на гарантии 3 и 5.
- [ ] **Step 5: Поправить спецификацию** — раздел 6: `common/space/*` больше не «в Default: да» целиком; указать шесть исключений и причину.
- [ ] **Step 6: Прогнать тесты** — Expected: PASS.
- [ ] **Step 7: Коммит** — `feat: пресеты Lebedev, Gost и Typograf разошлись с Default`.
...
- [ ] `rtk dotnet build -c Release` — без предупреждений.
- [ ] `RuleId.Registry.All.Length == 65`; `RuleId.TryParse` находит каждое имя из таблицы задачи 1.
- [ ] Ни одно правило не аллоцирует: путь `Process(ReadOnlySpan<char>, IBufferWriter<char>)` по-прежнему даёт 0 B в бенчмарке.
- [ ] Гарантии 3, 4, 5 держатся на расширенном корпусе `HardCases.All`.
- [ ] Пресеты `Lebedev`, `Gost`, `Typograf` отличаются от `Default` и друг от друга; отличия объяснены в XML-комментариях.
- [ ] Раздел 6 спецификации соответствует коду.
