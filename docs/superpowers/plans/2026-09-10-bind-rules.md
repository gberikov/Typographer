...
- [x] **Step 4: Маска фазы и пересечение**
...
- [x] **Step 1: Красный тест на инварианты новой модели**
...
- [x] **Step 5: Существующие три правила переезжают в `NbspRules`**
...
- [x] **Step 6: Конвейер отдаёт фазе буфер фазы Prepare**
...
- [x] **Step 1: Красные тесты** — четыре случая из таблицы, `[Fact]` на каждый; неразрывный
      пробел в тесте только через `Chars.Nbsp`.
- [x] **Step 2: Прогнать** — Expected: FAIL на `delBOM` при выключенном правиле (метка
      удаляется безусловно) и на обоих случаях `replaceNbsp`.
- [x] **Step 3: Реализация** — в `Preparer.Run` условие `isDocumentStart` дополняется
      проверкой правила; в цикле добавляется ветка замены `Chars.Nbsp` на пробел.
- [x] **Step 4: Прогнать** — Expected: PASS.
- [x] **Step 5: Коммит** — `feat: правила delBOM и replaceNbsp фазы Prepare`.
...
- [x] **Step 1: Красный тест** — `[Theory]` на каждое правило по строкам таблицы.
- [x] **Step 2: Прогнать** — Expected: FAIL, склеек нет.
- [x] **Step 3: Три словаря** в `Dictionaries` по образцу `IsYearAbbreviation`.
...
- [x] **Step 4: Три ветви** в условии `bindBack` задачи 4.
- [x] **Step 5: Прогнать** — Expected: PASS.
- [x] **Step 6: Коммит** — `feat: неразрывный пробел перед «млн», «руб.» и «dpi»`.
...
- [x] **Step 1: Красный тест** — `[Theory]` по строкам таблицы, по правилу на метод.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Четыре словаря.** `AddressAbbr`: `г.`, `обл.`, `край`, `р-н`, `ул.`, `пр.`,
...
- [x] **Step 4: Ветвь склейки вперёд** в `NbspRules.TryGlue`.
- [x] **Step 5: Прогнать** — Expected: PASS. Проверить, что `с.` в двух словарях сразу
      (село и страница) не приводит к разному поведению — действие у правил одно.
- [x] **Step 6: Коммит** — `feat: неразрывный пробел после адресных и ссылочных сокращений`.
...
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Два словаря.** `Particles`: `ли`, `ль`, `же`, `ж`, `бы`, `б`.
...
- [x] **Step 4: Две ветви** в `TryGlue`.
- [x] **Step 5: Прогнать** — Expected: PASS.
- [x] **Step 6: Коммит** — `feat: неразрывный пробел вокруг частиц и служебных слов`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы плюс случай `и т. д.`.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Две ветви** с общим предикатом `IsSentenceEnd(char boundary, ReadOnlySpan<char> token)`.
- [x] **Step 4: Прогнать** — Expected: PASS; тесты корпуса обновить по глазам.
- [x] **Step 5: Коммит** — `feat: неразрывный пробел перед последним коротким словом`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы; узкий пробел только через
      `Chars.NarrowNbsp`, в `[InlineData]` — ` `.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: `MarkRules`** и вызов из `BindSegment` перед записью границы.
- [x] **Step 4: Прогнать** — Expected: PASS. Отдельно прогнать `IdempotencyTests`: правило
      со вставкой — главный источник разрывов идемпотентности.
- [x] **Step 5: Коммит** — `feat: узкий неразрывный пробел после знаков номера и параграфа`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы плюс случай с тегом внутри токена.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Четыре правила** в `RewriteRules` c общим помощником
      `Replace(ref CharBuffer buffer, ref BindState state, Span<char> token, int from, ReadOnlySpan<char> replacement)`.
- [x] **Step 4: Прогнать** — Expected: PASS. Проверить `IdempotencyTests`: `гг.` не должно
      превращаться в `г.` на втором прогоне.
- [x] **Step 5: Коммит** — `feat: «гг.», «вв.», «P. S.» и квадратные метры`.
...
- [x] **Step 1: Красный тест** — три строки таблицы плюс случай `MaxNobr = 3` вместе с
      правилом (цепочки, созданные опцией, остаются со неразрывными пробелами).
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Счётчик глубины** в `RunDocument` по образцу `LayoutWriter.UpdateElementDepth`
      (разбор имени тега уже написан — вынести в общий помощник, а не копировать).
- [x] **Step 4: Прогнать** — Expected: PASS.
- [x] **Step 5: Коммит** — `feat: снятие неразрывных пробелов внутри nobr и nowrap`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы; отрицательных случаев больше, чем
      положительных, и это нормально для правила, меняющего данные.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: `DateRules`** и вызов из `FlushToken`.
- [x] **Step 4: Прогнать** — Expected: PASS, включая `HardCases` (там уже есть строка
      `01.01.2020` и `192.168.0.1`).
- [x] **Step 5: Коммит** — `feat: ISO-дата и регистр названий месяцев и дней недели`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: `MoneyRules`** плюс ветвь `ruble` в `RewriteRules`.
- [x] **Step 4: Прогнать** — Expected: PASS, отдельно `IdempotencyTests` с включённым
      набором `RuleSet.All`.
- [x] **Step 5: Коммит** — `feat: символ валюты после числа и знак рубля`.
...
- [x] **Step 1: Красный тест** — по строкам таблицы плюс `раз <b>раз</b>` (повтор через
      тег не удаляется).
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Две ветви** в `RewriteRules`.
- [x] **Step 4: Прогнать** — Expected: PASS.
- [x] **Step 5: Коммит** — `feat: ударение и удаление повтора слова`.
...
- [x] **Step 1: Красный тест** — все одиннадцать строк таблицы.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: `PhoneRules`** — чтение вперёд с накоплением до одиннадцати цифр в
      `Span<char> digits = stackalloc char[12]`, запись канонической формы, `state.Skip`.
- [x] **Step 4: Прогнать** — Expected: PASS; отдельно `RobustnessTests` (правило читает
      вперёд и обязано не выходить за `end`).
- [x] **Step 5: Коммит** — `feat: форматирование телефонных номеров`.
...
- [x] **Step 1: Красные тесты на пресеты** — по одному `[Fact]` на строку таблицы правок.
- [x] **Step 2: Прогнать** — Expected: FAIL.
- [x] **Step 3: Правка `RuleSet`** и XML-комментариев пресетов: комментарий обязан назвать
      причину каждого расхождения, а не только перечислить правила.
- [x] **Step 4: Прогон всех 36 строк снимка оракула через `Default`** и обновление раздела
...
- [x] **Step 5: Корпус** — `HardCases` пополняется строками из таблиц задач 4–15, которые
      ловят взаимодействие правил: `ул. Ленина, д. 5, кв. 7`, `в 3 г. IV в. до н. э.`,
      `+7 (999) 123-45-67`, `2026-09-10`, `100 руб.`, `№ 5 и § 3`.
- [x] **Step 6: Прогнать всё** — `dotnet test`. Expected: PASS.
- [x] **Step 7: Коммит** — `feat: пресеты Lebedev и Typograf учли правила фазы Bind`.
