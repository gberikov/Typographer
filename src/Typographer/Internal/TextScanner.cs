...
internal static class TextScanner
{
    ...
    public static void Run(ReadOnlySpan<char> source, RuleSet rules, ref ScanState state, ref CharBuffer buffer)
    {
        ...
        for (int i = 0; i < source.Length; i++)
        {
            ...
            // Границы веб-адреса — см. Url. Флаг вместо взгляда назад по буферу: искать
            // «://» от каждого знака препинания до ближайшего пробела — это O(n²) на
            // входе без пробелов, а гарантия 1 обещает один линейный проход.
            state.InsideUrl = state.InsideUrl ? !Url.Ends(source, i, previous) : Url.Starts(source, i);

            // Внутри веб-адреса не работает НИ ОДНО правило. Адрес — машинный
            // идентификатор, а не текст: точка в «?v=1.5» не десятичный разделитель,
            // «/1/2» не половина, дефис в «/2020-2026» не тире, «!=» не знак неравенства,
            // а «(» не начало «(c)». Любая такая правка меняет, куда ведёт ссылка, и при
            // включённом common/html/url испорченный адрес попадает ещё и в href.
            // Запрет стоит здесь, а не в каждом правиле: правил, читающих цифру и знак
            // препинания, полтора десятка, и защита в одном из них защитой не является.
            if (state.InsideUrl)
            {
                buffer.Write(c);
                continue;
